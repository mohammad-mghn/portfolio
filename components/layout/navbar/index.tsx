...
function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-6 left-[50%] translate-x-[-50%] h-14 w-full max-w-[calc(100%-1.75rem)] md:max-w-[calc(100%-6rem)] xl:max-w-[80rem] px-3 mx-auto flex items-center justify-between rounded-lg border-2 border-border-darker z-10 ${styles.box}`}
      >
        ...
        <Link href={"/fa"} className="mr-2 w-5 hidden md:block">
          <svg
            width="30"
            height="30"
            viewBox="0 0 30 30"
        ...
        <button title="menu" className="md:hidden" onClick={responsiveNavbarHandler}>
